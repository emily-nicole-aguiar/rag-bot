...
import datetime  # Para timestamp no histórico
...
MODEL = genai.GenerativeModel('gemini-2.5-flash')  # Modelo gratuito (corrigido para versão estável)
...
collection = client.get_or_create_collection(name="schema_collection")  # Para schemas
history_collection = client.get_or_create_collection(name="history_collection")  # Para histórico de perguntas
...
# Função para consultar RAG usando ChromaDB com TF-IDF (100% local) - atualizada com histórico
def query_rag(user_question, n_results=2):
    ...
    # Busca histórico relevante (últimas 2 queries similares) - corrigido: use [0] para acessar a lista interna
    history_results = history_collection.query(
        query_embeddings=query_vec,
        n_results=2
    )
    history_relevant = "\n".join(history_results['documents'][0]) if history_results['documents'] and len(history_results['documents'][0]) > 0 else ""
    
    # Combina: Schema + Histórico (para contexto na LLM)
    full_context = f"Schema relevante:\n{schema_relevant}\n\nHistórico recente de perguntas semelhantes:\n{history_relevant}"
    return full_context

# Função para armazenar pergunta + resposta no RAG histórico - corrigido: embeddings=embedding (já é list of list)
def store_history(user_question, sql_query, sql_results, final_answer):
    # Cria um documento com o histórico completo
    history_doc = f"""
    Pergunta do usuário: {user_question}
    SQL gerado: {sql_query}
    Resultados SQL (resumo): {sql_results[:500] + '...' if len(sql_results) > 500 else sql_results}  # Limita tamanho
    Resposta final: {final_answer}
    """
    
    # Vetoriza com o mesmo TF-IDF (ou crie um vectorizer separado se preferir)
    try:
        vectorizer = load_tfidf()  # Reusa o de schemas
    except FileNotFoundError:
        print("TF-IDF não encontrado; pule histórico.")
        return
    
    embedding = vectorizer.transform([user_question]).toarray().tolist()  # Embedding baseado na pergunta - já é [[floats]]
    
    # Adiciona à coleção de histórico (ID único, ex: timestamp)
    history_id = f"query_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"
    
    history_collection.add(
        documents=[history_doc],
        embeddings=embedding,  # Corrigido: direto, sem [embedding]
        ids=[history_id]
    )
    print(f"Histórico armazenado com ID: {history_id}")

# Função para gerar SQL via LLM (Gemini) - atualizada para contexto completo
def generate_sql(user_question, relevant_context):
    ...
    Use o histórico recente para manter consistência e evitar erros repetidos.
    
    Contexto completo (schema + histórico recente):
    {relevant_context}
...
# Função para executar SQL no SQL Server local - corrigido: 'loop' -> 'conn'
def execute_sql(sql_query):
    try:
        conn = pyodbc.connect(conn_str)
        df = pd.read_sql(sql_query, conn)  # Corrigido: 'loop' para 'conn'
        conn.close()
...
# Pipeline principal - atualizada com histórico
def rag_pipeline(user_question):
    # 1. Consultar RAG para contexto relevante (schemas + histórico)
    relevant_context = query_rag(user_question)
    
    # 2. Gerar SQL via LLM
    sql_query = generate_sql(user_question, relevant_context)
